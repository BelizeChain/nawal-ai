...
class WebSearchTool(AbstractTool):
    ...
    _SPEC = ToolSpec(
        ...
        parameters  = {
            ...
            "num_results": {"type": "integer", "description": "Max results (default 5)",   "required": False},
    ...
    def run(
        ...
        **_: Any,
        ...
        return ToolResult(
            ...
            output    = results,
            metadata  = {"latency_ms": latency_ms, "num_results": len(results), "query": query},
        )
    ...
    def _live_search(self, query: str, num_results: int, language: str) -> List[Dict[str, str]]:
        ...
        req = urllib.request.urlopen(url, timeout=self._timeout)  # noqa: S310 — internal service
