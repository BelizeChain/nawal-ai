...
class ByzantineDetector:
    ...
    def detect_anomalies(
        ...
        - Gradient norm Z-score (detect magnitude outliers)
        - Cosine similarity (detect direction outliers)

        Args:
            client_updates: Client model updates
            threshold_std: Z-score threshold for anomaly detection
            cosine_threshold: Minimum average cosine similarity to peers
        ...
        for i, norm in enumerate(norms):
            ...
            # Check 1: Norm Z-score
            z_score = abs(norm - mean_norm) / (std_norm + 1e-6)
            if z_score > threshold_std:
                reasons.append(f"norm_z_score={z_score:.2f}")

            # Check 2: Cosine similarity to peers
            if num_clients > 1:
                cosine_sims = []
                for j in range(num_clients):
                    ...
                    sim = torch.nn.functional.cosine_similarity(
                        flat_updates[i].unsqueeze(0),
                        flat_updates[j].unsqueeze(0),
                    ).item()
                    cosine_sims.append(sim)
                avg_cosine = sum(cosine_sims) / len(cosine_sims)
                if avg_cosine < cosine_threshold:
                    reasons.append(f"avg_cosine={avg_cosine:.3f}")
