...
class ByzantineRobustStrategy:
    """
    Byzantine-robust aggregation — delegates to security.byzantine_detection.ByzantineDetector.

    Protects against malicious participants using Krum, Multi-Krum,
    Trimmed Mean, Median, or PHOCAS aggregation.
    """
    ...
    def __init__(
        ...
        Args:
            method: Aggregation method from security module
            num_byzantine: Expected number of Byzantine clients
            trim_ratio: Fraction to trim (for TRIMMED_MEAN)
        """
    ...
    async def aggregate(
        ...
        """
        Aggregate using the security module's ByzantineDetector.
        ...
        if len(updates) < 3:
            logger.warning(
                "Too few updates for Byzantine-robust aggregation, "
                "falling back to FedAvg (minimum 3 required, got {})",
                len(updates),
            )
            fedavg = FedAvgStrategy()
            return await fedavg.aggregate(updates, current_weights)

        # Convert ModelUpdate list to format expected by ByzantineDetector
        client_updates = [update.weights for update in updates]
        client_ids = list(range(len(updates)))

        aggregated = self.detector.aggregate(
        ...
        logger.info(
            ...
            num_updates=len(updates),
            method=self.detector.method.value,
        )
...
class FederatedAggregator:
    ...
    def __init__(
        ...
        Args:
            ...
            participant_manager: Optional ParticipantManager for identity verification
        """
    ...
    async def submit_update(self, update: ModelUpdate) -> bool:
        ...
        if self.participant_manager is not None:
            ...
            if participant is None:
                ...
                return False
            if not participant.is_active(self.participant_manager.activity_timeout):
                logger.warning(
                    "Update from inactive participant",
                    participant_id=update.participant_id,
        ...
        if len(self.pending_updates[update.round_number]) >= self.min_participants:
            # Trigger aggregation with error handling
            task = asyncio.create_task(self._aggregate_round(update.round_number))
            task.add_done_callback(self._handle_aggregation_result)

        return True

    def _handle_aggregation_result(self, task: asyncio.Task) -> None:
        """Handle aggregation task completion — log exceptions instead of losing them."""
        try:
            task.result()
        except Exception as e:
            logger.error(
                f"Aggregation round failed: {e}",
                exc_info=True,
            )
