...
@pytest.mark.asyncio
async def test_gradient_inversion_low_magnitude_high_score(
    ...
    assert (
        score >= 95.0
    ), f"Low early-layer gradients should score >=95, got {score:.2f}"


@pytest.mark.asyncio
async def test_gradient_inversion_high_magnitude_low_score(
    training_config, simple_genome
):
    trainer = GenomeTrainer(training_config)
    trainer.set_genome(simple_genome)
    ...
    assert (
        score <= 30.0
    ), f"High early-layer gradients should score <=30, got {score:.2f}"


@pytest.mark.asyncio
async def test_information_leakage_healthy_confidence_high_score(
    training_config, simple_genome
):
    trainer = GenomeTrainer(training_config)
    trainer.set_genome(simple_genome)
...
@pytest.mark.asyncio
async def test_information_leakage_overconfident_low_score(
    training_config, simple_genome
):
    trainer = GenomeTrainer(training_config)
    trainer.set_genome(simple_genome)
    ...
    assert (
        60.0 <= score <= 75.0
    ), f"Overconfident predictions should score 60-75, got {score:.2f}"
...
async def test_prediction_confidence_history_limits(training_config, simple_genome):
    ...
    assert (
        len(trainer.prediction_confidences) == 100
    ), "Should retain last 100 confidences"
...
async def test_training_validation_history_tracking(training_config, simple_genome):
    ...
    assert (
        len(trainer.training_losses) == 50
    ), "Training loss history should be capped at 50"
    assert (
        len(trainer.validation_losses) == 50
    ), "Validation loss history should be capped at 50"
...
async def test_verify_data_leakage_integration(training_config, simple_genome):
    ...
    grads = {
        "00.weight": torch.randn(10, 10) * 0.1,
        "01.weight": torch.randn(10, 10) * 0.2,
    }
