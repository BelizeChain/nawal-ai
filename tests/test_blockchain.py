...
class TestStakingConnector:
    ...
    def connector(self):
        ...
        return StakingConnector(node_url="ws://localhost:9944", mock_mode=True)
    ...
    async def test_enroll_participant_success(self, connector, mock_keypair):
        ...
        success = await connector.enroll_participant(
            account_id=account_id, stake_amount=stake_amount, keypair=mock_keypair
        )
    ...
    async def test_submit_training_proof_success(self, connector, mock_keypair):
        ...
        await connector.enroll_participant(
            account_id, dalla_to_planck(2000), mock_keypair
        )
        ...
        success = await connector.submit_training_proof(submission, mock_keypair)
    ...
    async def test_claim_rewards_success(self, connector, mock_keypair):
        ...
        account_id = "dave"
        await connector.enroll_participant(
            account_id, dalla_to_planck(4000), mock_keypair
        )
    ...
    async def test_unenroll_participant(self, connector, mock_keypair):
        ...
        account_id = "eve"
        await connector.enroll_participant(
            account_id, dalla_to_planck(1000), mock_keypair
        )
...
class TestRewardCalculator:
    ...
    def test_calculate_overall_fitness(self, calculator):
        ...
        fitness = FitnessScores(quality=80.0, timeliness=90.0, honesty=85.0)
    ...
    def test_calculate_reward_perfect_performance(self, calculator):
        ...
        fitness = FitnessScores(quality=100.0, timeliness=100.0, honesty=100.0)
    ...
    def test_calculate_reward_average_performance(self, calculator):
        ...
        fitness = FitnessScores(quality=75.0, timeliness=80.0, honesty=70.0)
    ...
    def test_calculate_reward_poor_performance(self, calculator):
        ...
        fitness = FitnessScores(quality=30.0, timeliness=40.0, honesty=50.0)
    ...
    def test_estimate_monthly_rewards(self, calculator):
        ...
        monthly = calculator.estimate_monthly_rewards(
            rounds_per_day=4, avg_fitness=80.0, stake_amount_dalla=5000.0
        )
...
class TestBlockchainEventListener:
    ...
    def listener(self):
        ...
        return BlockchainEventListener(node_url="ws://localhost:9944", mock_mode=True)
    ...
    async def test_different_event_types(self, listener):
        ...
        listener.register_handler(
            EventType.TRAINING_PROOF_SUBMITTED, lambda e: training_events.append(e)
        )
        listener.register_handler(
            EventType.REWARDS_CLAIMED, lambda e: reward_events.append(e)
        )

        # Emit different events
        await listener.emit_mock_event(
            EventType.TRAINING_PROOF_SUBMITTED, {"participant_id": "bob", "round_id": 1}
        )
        await listener.emit_mock_event(
    ...
    async def test_event_history_tracking(self, listener):
        ...
        for i in range(5):
            await listener.emit_mock_event(
                EventType.TRAINING_ROUND_STARTED, {"round_id": i + 1}
            )
...
class TestBlockchainIntegration:
    ...
    async def test_full_training_cycle(self, setup):
        ...
        listener.register_handler(EventType.TRAINING_ROUND_STARTED, on_training_start)
        ...
        proof_submitted = await connector.submit_training_proof(submission, keypair)
    ...
    async def test_multiple_validators_competition(self, setup):
        ...
        for participant_id, stake in validators:
            await connector.enroll_participant(participant_id, stake, keypair)
        ...
        for (participant_id, stake), fitness in zip(validators, fitness_scores):
            ...
            # Calculate reward
            reward = calculator.calculate_reward(participant_id, 1, fitness, stake)
