...
@pytest.mark.asyncio
async def test_proper_gradient_clipping_high_score(
    training_config, simple_genome, mock_dp_config
):
    ...
    assert (
        score >= 95.0
    ), f"Properly clipped gradients should score >=95, got {score:.2f}"


@pytest.mark.asyncio
async def test_unclipped_gradients_low_score(
    training_config, simple_genome, mock_dp_config
):
...
@pytest.mark.asyncio
async def test_inconsistent_clipping_detected(
    training_config, simple_genome, mock_dp_config
):
...
@pytest.mark.asyncio
async def test_clipping_history_tracking(
    training_config, simple_genome, mock_dp_config
):
    ...
    assert all(
        0.0 <= norm <= 20.0 for norm in trainer.gradient_clip_history
    ), "Norms should be reasonable"
...
@pytest.mark.asyncio
async def test_privacy_budget_healthy_high_score(
    training_config, simple_genome, mock_dp_config
):
    ...
    assert (
        score >= 95.0
    ), f"Healthy budget (90% remaining) should score >=95, got {score:.2f}"


@pytest.mark.asyncio
async def test_privacy_budget_exhausted_low_score(
    training_config, simple_genome, mock_dp_config
):
...
@pytest.mark.asyncio
async def test_privacy_budget_near_limit_warning(
    training_config, simple_genome, mock_dp_config
):
    ...
    assert (
        40.0 <= score <= 80.0
    ), f"Near-exhausted budget should score 40-80, got {score:.2f}"
...
@pytest.mark.asyncio
async def test_proper_noise_consistency_high_score(
    training_config, simple_genome, mock_dp_config
):
    ...
    for _ in range(10):
        trainer._store_noise_scale(
            expected_noise + torch.randn(1).item() * 0.05
        )  # Small variance
...
@pytest.mark.asyncio
async def test_insufficient_noise_detected(
    training_config, simple_genome, mock_dp_config
):
...
async def test_noise_history_tracking(training_config, simple_genome):
    ...
    assert trainer.noise_scale_history[0] == pytest.approx(
        1.2, abs=0.01
    ), "Should keep recent values"
...
@pytest.mark.asyncio
async def test_honesty_score_integrates_dp(
    training_config, simple_genome, mock_dp_config
):
...
@pytest.mark.asyncio
async def test_gradient_clip_history_limits(
    training_config, simple_genome, mock_dp_config
):
...
@pytest.mark.asyncio
async def test_privacy_spent_history_limits(
    training_config, simple_genome, mock_dp_config
):
