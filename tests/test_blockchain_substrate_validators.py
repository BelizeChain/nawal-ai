...
class TestExtrinsicReceiptProperties:
    ...
    def test_error_message_alias(self):
        ...
        r = ExtrinsicReceipt(
            extrinsic_hash="0xdef", success=False, error="Module error"
        )
...
class TestValidatorManager:
    def _manager(self):
        ...
        mock_client.submit_extrinsic.return_value = MagicMock(success=True, error=None)
...
class TestBlockchainEventListenerMock:
    ...
    def test_dispatch_event_calls_handler(self):
        ...
        async def run():
            await listener.emit_mock_event(
                EventType.GENOME_EVOLVED, data={"genome_id": "g1"}
            )
