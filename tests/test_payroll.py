...
class TestPayrollConnector:
    ...
    def test_tax_calculation_above_deduction(self):
        """Test tax calculation above standard deduction."""
        connector = PayrollConnector(mock_mode=True)

        # Annual salary: 26,500 DALLA (500 above 26,000 deduction)
        gross_salary = 2650000000000
        tax = connector.calculate_tax_withholding(gross_salary)

        # Belize Income Tax Act: 25% of (26,500 - 26,000) = 25% of 500 = 125 DALLA
        expected_tax = 12500000000  # 125 DALLA in Planck
        assert abs(tax - expected_tax) < 100000000  # Allow small rounding error

    def test_tax_calculation_high_income(self):
        """Test tax calculation for high income."""
        connector = PayrollConnector(mock_mode=True)

        # Annual salary: 50,000 DALLA
        gross_salary = 5000000000000
        tax = connector.calculate_tax_withholding(gross_salary)

        # Belize Income Tax Act: 25% of (50,000 - 26,000) = 25% of 24,000 = 6,000 DALLA
        expected_tax = 600000000000  # 6,000 DALLA in Planck
