...
class GenomeTrainer:
    ...
    def __init__(
        ...
        self.prediction_history: list[dict[str, torch.Tensor]] = (
            []
    ...
    def set_genome(
        self, genome: Genome, initial_weights: dict[str, torch.Tensor] | None = None
    ) -> None:
        """
        Set genome to train.
        ...
        # Load initial weights if provided (with integrity validation)
        if initial_weights:
            for name, param in initial_weights.items():
                if torch.isnan(param).any() or torch.isinf(param).any():
                    raise ValueError(
                        f"Rejecting initial weights: parameter '{name}' contains "
                        "NaN or Inf values (possible model poisoning)"
                    )
                if param.abs().max() > 1e6:
                    logger.warning(
                        f"Initial weight '{name}' has extreme magnitude: "
                        f"{param.abs().max().item():.2e}"
                    )
            self.current_model.load_state_dict(initial_weights)
            logger.info("Loaded initial weights from server (integrity validated)")
    ...
    def train(
        ...
        **kwargs,
        ...
        for epoch in range(start_epoch, start_epoch + num_epochs):
            ...
            logger.info(
                f"Epoch {epoch + 1}/{start_epoch + num_epochs} - Loss: {train_metrics['loss']:.4f}"
            )
    ...
    def _calculate_honesty_score(
        ...
        try:
            ...
            logger.error(f"Gradient norm verification failed: {e}")
            scores.append(("gradient_norms", 0.0))  # Fail-closed: assume worst
        ...
        try:
            ...
            scores.append(("byzantine", 0.0))  # Fail-closed: assume worst
        ...
        try:
            ...
            logger.error(f"Magnitude verification failed: {e}")
            scores.append(("magnitude", 0.0))  # Fail-closed: assume worst
        ...
        if predictions is not None or losses or activations:
            try:
                ...
                scores.append(("data_poisoning", 0.0))  # Fail-closed: assume worst
        ...
        if (
            (self.training_losses or self.validation_losses)
            or predictions is not None
            or gradients is not None
        ):
            try:
                leakage_score = self._verify_data_leakage(
                    gradients=gradients, predictions=predictions
                )
                scores.append(("data_leakage", leakage_score))
            except Exception as e:
                logger.error(f"Data leakage verification failed: {e}")
                scores.append(("data_leakage", 0.0))  # Fail-closed: assume worst
    ...
    def validate_privacy_compliance(self, gradients: dict[str, torch.Tensor]) -> bool:
        ...
        model_size_mb = (
            self.current_model.get_memory_footprint() / 1024**2
            if self.current_model is not None
            else None
        )
        if model_size_mb is not None and model_size_mb < 10.0:  # Small models
            for name, grad in gradients.items():
                if grad is None:
                    continue
                ...
                if grad_norm > 10.0:
                    logger.warning(
                        f"Membership inference risk: Small model ({model_size_mb:.1f}MB) "
    ...
    def _check_feature_distribution(
        ...
        for layer_name, current_act in activations.items():
            ...
            hist_mean_std = (
                sum((m - hist_mean_avg) ** 2 for m in hist_means) / len(hist_means)
            ) ** 0.5
    ...
    def _check_noise_consistency(self, dp_config: Any) -> float:
        ...
        for noise_scale in recent_noise:
            relative_deviation = abs(noise_scale - expected_noise_scale) / (
                expected_noise_scale + 1e-6
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_loss": sum(m.train_loss for m in self.training_metrics)
            / len(self.training_metrics),
            "avg_quality": sum(m.quality_score for m in self.training_metrics)
            / len(self.training_metrics),
            "avg_timeliness": sum(m.timeliness_score for m in self.training_metrics)
            / len(self.training_metrics),
            "avg_honesty": sum(m.honesty_score for m in self.training_metrics)
            / len(self.training_metrics),
