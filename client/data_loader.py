    ...
    def filter_batch(
        ...
        for i, text in enumerate(texts):
            if not self._is_compliant(text):
                ...
                compliant_indices.append(i)

        # Remove non-compliant items from batch
        if len(compliant_indices) < len(texts):
            for key in batch:
                if isinstance(batch[key], torch.Tensor) and len(batch[key]) > 0:
...
class BelizeDataLoader:
    ...
    def _load_participant_data(self):
        ...
        if not data_dir.exists():
            logger.warning(
                f"No training data found for participant {self.participant_id} "
                f"at {data_dir} — generating synthetic data. This produces "
                "MEANINGLESS model updates and should NOT be used in production. "
                "Set NAWAL_REQUIRE_REAL_DATA=true to fail instead."
            )
            if os.getenv("NAWAL_REQUIRE_REAL_DATA", "").lower() == "true":
                raise FileNotFoundError(
                    f"Training data directory {data_dir} not found and "
                    "NAWAL_REQUIRE_REAL_DATA=true"
                )
