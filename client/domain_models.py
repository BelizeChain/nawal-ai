...
class AgriTechModel(DomainModel):
    ...
    def _preprocess_sensor_data(self, sensor_data: bytes) -> torch.Tensor:
        ...
        if len(sensor_data) % itemsize != 0:
            raise ValueError(
                f"Sensor data length {len(sensor_data)} not aligned to float32 "
                f"(itemsize={itemsize})"
            )
...
class MarineModel(DomainModel):
    ...
    def _preprocess_water_quality(self, sensor_data: bytes) -> torch.Tensor:
        ...
        if len(sensor_data) == 0:
            raise ValueError("Empty water quality sensor data")
        itemsize = np.dtype(np.float32).itemsize
        if len(sensor_data) % itemsize != 0:
            raise ValueError(
                f"Water quality data length {len(sensor_data)} not aligned to "
                f"float32 (itemsize={itemsize})"
            )
        readings = np.frombuffer(sensor_data, dtype=np.float32)
        if not np.all(np.isfinite(readings)):
            raise ValueError("Water quality data contains NaN or Inf values")
...
class EducationModel(DomainModel):
    ...
    def __init__(
        ...
        model_name = getattr(config, "model_name", None) or (
            config.tokenizer_name if config else "character"
        )
...
class TechModel(DomainModel):
    ...
    def _preprocess_metrics(self, data: bytes) -> torch.Tensor:
        ...
        if not np.all(np.isfinite(metrics)):
            raise ValueError("Metrics data contains NaN or Inf values")
...
class DomainModelFactory:
    ...
    def create_model(
        ...
        model_classes = {
            ...
            ModelDomain.GENERAL: AgriTechModel,  # DESIGN NOTE: GeneralModel TBD; AgriTech used as interim default
        }
