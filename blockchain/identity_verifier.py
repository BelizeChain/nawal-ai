...
def create_verifier(mode: str = "production", **kwargs) -> BelizeIDVerifier:
    ...
    Raises:
        RuntimeError: If attempting to use development mode in production environment
    """
    env = os.environ.get("NAWAL_ENV", "production").lower()

    if mode == "development":
        if env == "production":
            raise RuntimeError(
                "DummyBelizeIDVerifier cannot be used when NAWAL_ENV=production. "
                "Set NAWAL_ENV=development to use the dummy verifier."
            )
        logger.warning(
            "⚠️ Using DummyBelizeIDVerifier — identity verification is DISABLED. "
